...
// Make create a TextContainer from native pointer
func MakeTextContainer(ptr unsafe.Pointer) *NSTextContainer {
	return &NSTextContainer{*foundation.MakeObject(ptr)}
}

// New create new TextContainer
func NewTextContainer(size foundation.Size) TextContainer {
	id := resources.NextId()
	ptr := C.TextContainer_New(C.long(id), toNSSize(size))
